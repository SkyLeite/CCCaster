...
static std::string SteamIdToString(const CSteamID &id)
{
    // 64-bit “SteamID64” representation
    uint64_t x = id.ConvertToUint64();

    char buf[32];
    // Use PRIu64 for portable formatting
    std::snprintf(buf, sizeof(buf), "%llu", x);
...
vector<string> Lobby::getMenu()
{
    if (mode == MENU)
    {
        return {"Public Lobbies", "Create Lobby", "Enter Lobby Code", "Default Lobby", "Steam Lobbies"};
    }
    ...
    else if (mode == STEAM_LOBBY)
    {
        return steamlobbyentries;
    }
    else
    {
        return entries;
...
void Lobby::fetchSteamLobby()
{
    LOG("Fetching Steam lobbies");
    entryMutex.lock();
    auto lobby_list_api_call = SteamMatchmaking()->RequestLobbyList();
    m_CallResultLobbyMatchList.Set(lobby_list_api_call, this, &Lobby::OnLobbyMatchList);
}

void Lobby::OnLobbyMatchList(LobbyMatchList_t *pLobbyMatchList, bool bIOFailure)
{

    // LOG("Lobby list received: %d lobbies found", pLobbyMatchList->m_nLobbiesMatching);
    for (size_t i = 0; i < pLobbyMatchList->m_nLobbiesMatching; i++)
    {
        auto lobby_id = SteamMatchmaking()->GetLobbyByIndex(i);
        auto lobbyIdStr = SteamIdToString(lobby_id);
        // LOG("Found lobby %s", lobbyIdStr);
        steamlobbyentries.push_back(std::string(lobbyIdStr));
    }

    // LOG("Unlocking lobby mutex");
    entryMutex.unlock();
}
